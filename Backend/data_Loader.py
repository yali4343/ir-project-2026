...
def load_index(index_type):
    ...
    if index_source in ["gcs", "auto"]:
        if bucket_base_dir:
            print(
                f"Attempting to load index from GCS: gs://{Config.BUCKET_NAME}/{bucket_base_dir}/{name}.pkl"
            )
            try:
                return InvertedIndex.read_index(
                    bucket_base_dir, name, Config.BUCKET_NAME
                )
            ...
            if index_source == "gcs":
                ...
                raise ValueError(
                    f"No GCS path configured for index type '{index_type}'"
                )
...
def load_pagerank():
    ...
    if index_source in ["gcs", "auto"]:
        try:
            print(
                f"Loading PageRank from GCS: gs://{Config.BUCKET_NAME}/{Config.PAGERANK_CSV_GZ_GCS}"
            )
            bucket = get_bucket()
...
def load_id_to_title():
    ...
    if index_source in ["gcs", "auto"]:
        try:
            print(
                f"Loading id_to_title from GCS Parquet: gs://{Config.BUCKET_NAME}/{Config.ID_TO_TITLE_PARQUET_DIR_GCS}"
            )
            bucket = get_bucket()
