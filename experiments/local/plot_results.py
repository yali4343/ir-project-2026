...
def plot_metrics():
    """
    Parses all `metrics.json` files in the runs directory and generates comparison plots.

    Plots generated:
    1. performance_comparison.png: Grouped bar chart of P@10 and AP@10.
    2. latency_comparison.png: Bar chart of average latency.

    The function automatically discovers all subdirectories in RUNS_DIR.
    """
    # 1. Setup
    ...
    if len(latencies) > 0:
        rects3 = ax.bar(
            x_pos, latencies, width, color="#FFC107", label="Avg Latency (ms)"
        )
