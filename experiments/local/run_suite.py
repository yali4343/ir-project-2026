...
def run_suite(version_name, num_runs, seed_base, split_ratio):
    ...
    print(f"Mean P@10: {aggregate_data['mean_p10_avg']:.4f} (+/- {aggregate_data['mean_p10_std']:.4f})")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Experiment Suite (Multi-run aggregation)")
    parser.add_argument("--version_name", type=str, required=True, help="Label for this version (e.g. baseline, v2_weights)")
    parser.add_argument("--num_runs", type=int, default=3, help="Number of runs to average")
    parser.add_argument("--seed_base", type=int, default=42, help="Starting random seed")
