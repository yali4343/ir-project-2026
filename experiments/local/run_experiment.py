...
def load_queries(path):
    ...
    Args:
        path (str): Path to the queries JSON file.

    Returns:
        dict: Dictionary of query -> relevant_doc_ids.
    """
...
def calculate_metrics(relevant_ids, retrieved_ids, k=10):
    """
    Computes Precision@k and Average Precision@k.

    Args:
        relevant_ids (list): List of relevant document IDs (assignments).
        retrieved_ids (list): List of document IDs retrieved by the engine.
        k (int): Cut-off rank.

    Returns:
        tuple: (p_at_k, ap_at_k) float values.
    """
...
def run_experiment(experiment_name, seed, split_ratio, max_queries, output_dir):
    ...
    2. Splits queries into Train/Test (reuses seeded random).
    3. Runs the selected queries (Test set) against the engine.
    4. Computes P@10, AP@10, and Latency.
    5. Saves full logs and metrics to JSON.

    Args:
        experiment_name (str): Unique name for the run folder.
        seed (int): Random seed for reproducibility.
        split_ratio (float): Ratio to split train/test (e.g. 0.8).
        max_queries (int): Optional limit on number of queries to run (for smoke tests).
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--experiment_name", type=str, required=True, help="Name of the experiment run"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="experiments/local/runs",
