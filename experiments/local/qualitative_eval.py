...
def qualitative_eval(run_path, output_dir):
    """
    Generates a qualitative report for a specific experiment run.
    Identifies Best/Worst performing queries based on P@10 and AP@10.
    Fetches actual results for those queries and writes a Markdown report.

    Args:
        run_path (str): Path to the specific run directory containing per_query.jsonl.
        output_dir (str): Directory where the Markdown report will be saved.

    Side Effects:
        Writes qualitative_report.md to output_dir.
    """
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            f"**Metrics:** P@10: {best_res['p10']:.4f}, AP@10: {best_res['ap10']:.4f}\n\n"
        )
        ...
        f.write(
            f"**Metrics:** P@10: {worst_res['p10']:.4f}, AP@10: {worst_res['ap10']:.4f}\n\n"
        )
        f.write("| Rank | Doc ID | Title |\n")
        f.write("|---|---|---|\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to a specific run folder (e.g. experiments/local/runs/timestamp_name)",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="experiments/local/qualitative",
