...
def measure_latency(base_url, queries_path, num_queries, repeat, version_name):
    ...
    np.random.seed(42)
    selected_queries = np.random.choice(all_queries, size=min(len(all_queries), num_queries), replace=False)
    ...
    for i, q_text in enumerate(selected_queries):
        ...
        for _ in range(repeat):
            try:
                ...
                resp = session.get(f"{base_url}/search", params={'query': q_text}, timeout=30)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_url", type=str, required=True, help="Base URL (e.g., http://34.1.1.1:8080)")
    parser.add_argument("--num_queries", type=int, default=30, help="Number of queries to sample")
    parser.add_argument("--repeat", type=int, default=3, help="Repeats per query")
    parser.add_argument("--version_name", type=str, default="gcp_v1", help="Version label")
    
    # Try to find queries file automatically or take Arg
    default_q = os.path.join(DATA_DIR, "queries_train.json")
    parser.add_argument("--queries_file", type=str, default=default_q, help="Path to queries_train.json")
    
    args = parser.parse_args()
    
    measure_latency(args.base_url, args.queries_file, args.num_queries, args.repeat, args.version_name)
