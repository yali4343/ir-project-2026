...
def main():
    # 1. Download Body Index (Global Stats)
    ...
    # Note: This script only downloads the metadata (index.pkl).
    # To run efficiently, you should also download the .bin files.
    # We are not downloading all .bin files here to save time, 
    # but the search engine will try to read them from the bucket if missing locally 
    # (which will be slow but functional).
